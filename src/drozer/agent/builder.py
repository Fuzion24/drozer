...
class Packager(command_wrapper.Wrapper):
    ...
    def apktool_yaml_path(self):
        return os.path.join(self.__wd, "agent", self.__apktool_yaml)
    ...
    def package(self):
        ...
        if self._execute([self.__java, "-jar", self.__apk_tool, "-q", "build", self.source_dir(), "-o", self.apk_path(False)]) != 0:
